...
export default function ContactCard({ contactName, phone, email }) {
    ...
    return (
        <aside className="contactCard">
            ...
            <div className="contactCard__meta">
                איש קשר:{" "}
                <span className="contactCard__strong">{contactName || "לא צוין"}</span>
            </div>
            ...
            {hasEmail ? (
                <a className="contactCard__email" href={`mailto:${email}`}>
                ...
                <div className="contactCard__emailDisabled">אין אימייל זמין</div>
            )}
        </aside>
    );
}
